...
mps = read.csv("tables/mps_settings_long.csv", TRUE)
...
* Carrier Gas: Helium
* Transfer line Temperature (&deg;C): 250
...
* Temperature: Initial - 200&deg;C; duration 0 min

Oven:

* Equilibration time (s): 60
* Modulator enabled: yes
* Modulator offset (&deg;C): 5


```{r gradient, echo=FALSE, fig.cap="GC gradient - graphical representation. Rate in (&deg;C/min), Target temperature in (&deg;C), Duratin in (min).", fig.width=5}
...
kable(grad, booktabs = TRUE, caption = "GC gradient profile in numbers.")
